...
//        }
        ...
        {
            using (var bytes = MsgPackSerializer.Serialize(enumValue.ToString(), out var wroteSize))
            {
                var readOnlySequence = bytes.Memory.ToMultipleSegments().Slice(0, wroteSize);
                var enumResult = MsgPackSerializer.Deserialize<DefaultEnum>(readOnlySequence, out var readSize);
                enumResult.ShouldBe(enumValue);
                readSize.ShouldBe(wroteSize);
            }
        }
...
//        }
        ...
        public void ReadFlagEnumAsString(FlagEnum enumValue)
        {
            ...
            {
                var readOnlySequence = bytes.Memory.ToMultipleSegments().Slice(0, wroteSize);
                var enumResult = MsgPackSerializer.Deserialize<FlagEnum>(readOnlySequence, out var readSize);
                enumResult.ShouldBe(enumValue);
                readSize.ShouldBe(wroteSize);
            }
        }
