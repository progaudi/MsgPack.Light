...
//        }
        ...
        {
            using (var enumBlob = MsgPackSerializer.Serialize(enumValue, out var wroteSize))
            using (var stringBlob = MsgPackSerializer.Serialize(enumValue, out var stringSize))
                enumBlob.Memory.Slice(0, wroteSize).ShouldBe(stringBlob.Memory.Slice(0, stringSize));
        }
