...
namespace ProGaudi.MsgPack.Light.Benchmark
{
    ...
    public class BeerListDeserializeBenchmark
    {
        ...
        {
            _msgPack.Seek(0, SeekOrigin.Begin);
            var beer = MsgPackSerializer.Deserialize<Beer[]>(_msgPack, Serializers.MsgPackLightAutoGeneration);
        }

        [Benchmark]
        public void MPLightH_Array_AutoMap()
        {
            var beer = MsgPackSerializer.Deserialize<Beer[]>(_msgPackArray, Serializers.MsgPackLightAutoGeneration);
        }
    }
}